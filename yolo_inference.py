...
## SPECIFY THESE ##
DATASET_FOLDER = '../dataset/20240320_22_33_11_fps1_clip_1_0/'
...
def drawPredicted(classId, conf, left, top, right, bottom, color_frame, dpt_frame, x ,y):
    # print(classId, conf, x, y)
    cv2.rectangle(color_frame, (left,top), (right,bottom), (255,178,50),1)
    distance = np.mean(dpt_frame[y][x])
    cv2.circle(color_frame,(x,y),radius=1,color=(0,0,254), thickness=5)
    label = '%.2f' % conf
    ...
    distance_string = "Dist: " + str(distance/1000) + " meter away"
...
def process_detection(color_frame, depth_frame, outs, filename):
    ...
    with open(csv_path, 'w', newline='') as csvfile:
        ...
        writer.writerow(['Object Class Label', 'confidence', 'left', 'top', 'right', 'bottom'])
        ...
        for i in indices:
            ...
            drawPredicted(classIds[i], confidences[i],  left, top, left+width, top+height, color_frame, depth_frame, x, y)

            # Write the class labels, confidence, bbox to the CSV file
            writer.writerow([classes[classIds[i]], confidences[i], left, top, right, bottom])
